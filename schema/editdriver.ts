...
export const EditDriverSchema = v.object({
  ...
  email: v.optional(v.pipe(v.string(), v.trim(), v.union([v.literal(""), v.pipe(v.string(), v.email("Invalid email address"))]))),
  mobile: v.pipe(
    v.string(),
  ...
  experience: v.optional(v.union([v.literal(""), v.pipe(v.string(), v.regex(/^[0-9]+$/, "Experience must be a number"))])),
  joiningDate: v.optional(v.string()),
  salary: v.optional(v.union([v.literal(""), v.pipe(v.string(), v.regex(/^[0-9]+$/, "Salary must be a number"))])),
  ...
  // Status
  status: v.optional(v.picklist(["ACTIVE", "INACTIVE", "ON_LEAVE", "SUSPENDED"])),
});

export type EditDriverForm = v.InferOutput<typeof EditDriverSchema>;
